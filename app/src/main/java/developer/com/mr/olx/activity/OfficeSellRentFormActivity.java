...
import developer.com.mr.olx.ModelClasses.MultipartOfficeFormAPI;
...
public class OfficeSellRentFormActivity extends AppCompatActivity {
    String sub_title = "";
    TextView txt_title;
    ...
    Bitmap bitmap9;
    Bitmap bitmap10;
    EditText edt_carpet_area, edt_employee_capacity;
    Button btn_post;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.sell_rent_office_form);
        ...
        ll_emp_capacity = findViewById(R.id.ll_emp_capacity);
        ...
        edt_employee_capacity = findViewById(R.id.edt_employee_capacity);
        ...
        iv_add_image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (listPhoto.size() != 0 && listPhoto.size() > 10) {
                    ClsGlobal.errorMsg(OfficeSellRentFormActivity.this, v, "Select at least 10 photo...!");
                    return;
                }
                ClsGlobal.OnImageSelection(OfficeSellRentFormActivity.this, REQUEST_CODE_CHOOSE);
            }
        });

        mAdapter = new UriAdapter(OfficeSellRentFormActivity.this, listPhoto);
        ...
        btn_post.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (validation(view)) {
                    ...
                    Toast.makeText(OfficeSellRentFormActivity.this, "Fill required value...!", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    void selectCarTypeDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(OfficeSellRentFormActivity.this);
    ...
    void SellOfficeFillForm() {

        pd = ClsGlobal._prProgressDialog(this, "Adding new Product for sale...", false);
        pd.show();

        ClsOffice mObj = new ClsOffice();
        mObj.setOfficeInventoryId(0);
        mObj.setUserId(1);
        mObj.setCatId(cat_id);
        mObj.setSubCatId(sub_id);
        mObj.setoTitle(edt_title.getText().toString());
        mObj.setpTypeId(Integer.parseInt(txt_select_type.getTag().toString()));
        mObj.setpName(edt_property_name.getText().toString());
        mObj.setCabin(Integer.valueOf(txt_bedroom.getTag().toString()));
        mObj.setWashroom(Integer.valueOf(txt_bathroom.getTag().toString()));
        mObj.setoSqFt(Integer.parseInt(edt_sq_ft.getText().toString()));
        mObj.setoPrice(Double.valueOf(edt_price.getText().toString()));

        if (bitmap1 != null) {
            mObj.setoPictureLink1("data:image/png;base64," + ClsGlobal.getBase64Drawable(bitmap1));
        } else {
            mObj.setoPictureLink1("");
        }

        if (bitmap2 != null) {
            mObj.setoPictureLink2("data:image/png;base64," + ClsGlobal.getBase64Drawable(bitmap2));
        } else {
            mObj.setoPictureLink2("");
        }

        if (bitmap3 != null) {
            mObj.setoPictureLink3("data:image/png;base64," + ClsGlobal.getBase64Drawable(bitmap3));
        } else {
            mObj.setoPictureLink3("");
        }

        mObj.setoPictureLink4(baseUrlDemo);
        mObj.setoPictureLink5(baseUrlDemo);
        mObj.setoPictureLink6(baseUrlDemo);
        mObj.setoPictureLink7(baseUrlDemo);
        mObj.setoPictureLink8(baseUrlDemo);
        mObj.setoPictureLink9(baseUrlDemo);
        mObj.setoPictureLink10(baseUrlDemo);

        mObj.setpFurnishingId(Integer.parseInt(txt_furnishing.getTag().toString()));
        mObj.setoSupBuiltupArea(Integer.valueOf(edt_super_build_up_area.getText().toString()));
        mObj.setpCarParkingId(Integer.valueOf(txt_car_parking.getTag().toString()));
        mObj.setoMaintenance(Integer.valueOf(edt_maintenance.getText().toString()));
        mObj.setoShowMoNo(getMobileNoValBool);
        mObj.setoDescription(edt_description.getText().toString());
        mObj.setoLocation(edt_location.getText().toString());
        ...
        apiPostViewModel.FillOfficeForm(mObj).observe(this, obj -> {
            if (obj != null) {
...
//                    listPhoto.add(new clsPhoto((long) i3, selectedPhotos.get(i3)));
    ...
    void saveMultipleImages() {
        ...
        pd = ClsGlobal._prProgressDialog(OfficeSellRentFormActivity.this,
                "please wait for upload images...!", false);
        pd.show();
        for (int i3 = 0; i3 < this.listPhoto.size(); i3++) {
            bmpArray.add(listPhoto.get(i3).getmBitmap());
        }
        ClsGlobal.storeMultipleImage(OfficeSellRentFormActivity.this, bmpArray);
    }
    ...
    void uploadMultipleImageAPI(File mFile1, File mFile2, File mFile3, File mFile4, File mFile5,
        ...
        MultipartOfficeFormAPI multipleImageUploader = new MultipartOfficeFormAPI(OfficeSellRentFormActivity.this);
        multipleImageUploader.uploadFormDataFormAPI(0, 1, edt_title.getText().toString(),
                Integer.parseInt(txt_select_type.getTag().toString()), edt_property_name.getText().toString(),
                Integer.parseInt(txt_bedroom.getTag().toString()), Integer.parseInt(txt_bathroom.getTag().toString()),
                Double.parseDouble(edt_sq_ft.getText().toString()), Double.parseDouble(edt_price.getText().toString()),
                mFile1, mFile2, mFile3, mFile4, mFile5, mFile6, mFile7, mFile8, mFile9, mFile10,
                Integer.parseInt(txt_furnishing.getTag().toString()), Integer.parseInt(txt_car_parking.getTag().toString()),
                ...
                Double.parseDouble(edt_employee_capacity.getText().toString()),
                Double.parseDouble(edt_maintenance.getText().toString()),
                getMobileNoValInt, edt_description.getText().toString(),
        ...
        multipleImageUploader.SetCallBack(new MultipartOfficeFormAPI.FileUploaderCallback() {
            @Override
            public void onError() {
                if (pd.isShowing()) {
                    pd.dismiss();
            ...
            public void onFinish(ClsInventoryMasterFormData responses) {
                if (responses.getSuccess() == 1) {
                    ...
                    ClsGlobal.deleteMultipleImagesFromFolder(OfficeSellRentFormActivity.this);
                }
            }
